clc;clear all;


N=1000;
s = source(N); %信源产生，序列个数为N
s = source(N); %淇℃簮浜х敓锛屽簭鍒椾釜鏁颁负N
Eb = 1/2;
mu = 0;
SNR = 15;
N0 = Eb./(power(10,SNR/10));
sigma = sqrt(N0/2); %璁＄畻鍣０鐨勬爣鍑嗗樊

for i =1:length(sigma)
    n = normrnd(mu,sigma(i),2,N/2);   %浜х敓鏈嶄粠楂樻柉鍒嗗竷鐨勫弻璺櫔澹?
    n_c=n(1,:);n_s=n(2,:);
    s1_c=zeros(1,N/2);s1_s=zeros(1,N/2);

    for c=1:N/2
        s1_c(c)=s(2*c-1);
        s1_s(c)=s(2*c);
    end                     %灏嗕俊婧愬垎瑙ｆ垚鍙岃矾淇″彿
    
    [s_c1,s_s1] = QPSK(s1_c,s1_s);     %杩涜QPSK缂栫爜
    

    h = normrnd(0,sqrt(1/2),2,N/2);              %浜х敓鐟炲埄涔樻?鍣０
    h_i = h(1,:);h_q = h(2,:);
    s_c = s_c1.*h_i - s_s1.*h_q ;s_s = s_c1.*h_q + s_s1.*h_i;
    
    r_c = s_c  + n_c;r_s = s_s + n_s;
    figure(i+4)
    scatter(r_c,r_s)
    xlabel('In-phase');
    ylabel('Quadrature-phase');
    title(sprintf('Rayleigh,SNR = %d',SNR(i)));
end

